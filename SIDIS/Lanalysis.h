...
int Lanalysis::ENuclearPDF(const char bintree[], const char savefile[]){
  ...
  for (int nb = 0; nb < Nbinp; nb++){
    ...
    Tdata->SetBranchAddress("acc_pion_m", &acc_pion[1]);
    TH1D * h0 = new TH1D("h0", "h0", 1, 0.0, 2.0);
    TH1D * h1 = new TH1D("h1", "h1", 1, 0.0, 2.0);
    for (int ie = 0; ie < Nevent; ie++){
      Tdata->GetEntry(ie);
      if (Pt < Ptl || Pt > Ptu) continue;
      if (x < xl || x > xu) continue;
      Lstructure::sigmaUUTnlo(AZ, lab, sigma);
      h0->Fill(1.0, sigma[0]*acc_ele*acc_pion[0]);
      Lstructure::sigmaUUTreal(lab, sigma, "helium-3");
      h1->Fill(1.0, sigma[0]*acc_ele*acc_pion[0]);
    }
    Nfree = (h0->Integral(1, -1)) * _eff * _lumi * _days * 24.0 * 3600.0 / _simdensity;
    Nreal = (h1->Integral(1, -1)) * _eff * _lumi * _days * 24.0 * 3600.0 / _simdensity;
    ErrRel = std::abs(Nreal - Nfree) / Nreal; 
    Np->Fill();
    h0->Delete();
    h1->Delete();
    Tdata->Delete();
  ...
  for (int nb = 0; nb < Nbinm; nb++){
    ...
    for (int ie = 0; ie < Nevent; ie++){
      ...
      h0->Fill(1.0, sigma[1]*acc_ele*acc_pion[1]);
      Lstructure::sigmaUUTreal(lab, sigma, "helium-3");
      h1->Fill(1.0, sigma[1]*acc_ele*acc_pion[1]);
    }
    Nfree = (h0->Integral(1, -1)) * _eff * _lumi * _days * 24.0 * 3600.0 / _simdensity;
    Nreal = (h1->Integral(1, -1)) * _eff * _lumi * _days * 24.0 * 3600.0 / _simdensity;
    ErrRel = std::abs(Nreal - Nfree) / Nreal; 
