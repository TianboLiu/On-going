...
class Lanalysis{
 ...
 public:
  ...
  int BinAnalysisNeutronTriger(const char savefile[]);
...
int Lanalysis::BinAnalysisNeutronTriger(const char savefile[]){
  const double prescale = 10.0;
  const double degtorad = M_PI / 180.0;
  double factor = 1.0;
  TVector3 vec_e, vec_pi;
  TFile * fbinp = new TFile(_binfile1,"r");
  if (!fbinp->IsOpen()){
    std::cout << "Lanalysis::binanalysis: Bin info file does not exist!" << std::endl;
    return 1;
  ...
  for (int nb = 0; nb < Nbinp; nb++){
    ...
    TH1D * hsim = new TH1D("hsim", "hsim", 1, 0.0, M_PI);
    TH1D * h0p = new TH1D("h0p", "h0p", 180, 0.0, M_PI);
    TH1D * h1p = new TH1D("h1p", "h1p", 180, 0.0, M_PI);
    TH1D * hnp = new TH1D("hnp", "hnp", 1, 0.0, M_PI);
    TH1D * hxp = new TH1D("hxp", "hxp", 1, 0.0, M_PI);
    TH1D * hyp = new TH1D("hyp", "hyp", 1, 0.0, M_PI);
    TH1D * hzp = new TH1D("hzp", "hzp", 1, 0.0, M_PI);
    ...
    for (double ie = 0; ie < Nevent; ie = ie + 1){
      ...
      Lstructure::sigmaUUT(AZ, lab, sigmaN);
      Lstructure::sigmaUUTn(lab, sigman);
      vec_e.SetXYZ(lab[1] * sin(lab[2]*degtorad) * cos(lab[3]*degtorad), lab[1] * sin(lab[2]*degtorad) * sin(lab[3]*degtorad), lab[1] * cos(lab[2]*degtorad));
      vec_pi.SetXYZ(lab[4] * sin(lab[5]*degtorad) * cos(lab[6]*degtorad), lab[4] * sin(lab[5]*degtorad) * sin(lab[6]*degtorad), lab[4] * cos(lab[5]*degtorad));
      if (vec_e.Angle(vec_pi) / degtorad < 2.5) factor = 1.0 / prescale;
      else factor = 1.0;
      hsim->Fill(std::abs(kin[5]),1);
      h0p->Fill(std::abs(kin[5]), sigmaN[0]*acc_ele*acc_pion[0]);
      h1p->Fill(std::abs(kin[5]), sigmaN[0]*acc_ele*acc_pion[0]*factor);
      hnp->Fill(std::abs(kin[5]), sigman[0]*acc_ele*acc_pion[0]);
      hxp->Fill(std::abs(kin[5]), kin[0]*sigmaN[0]*acc_ele*acc_pion[0]);
      hyp->Fill(std::abs(kin[5]), kin[1]*sigmaN[0]*acc_ele*acc_pion[0]);
      hzp->Fill(std::abs(kin[5]), kin[2]*sigmaN[0]*acc_ele*acc_pion[0]);
    ...
    Nsim = hsim->Integral(1, -1);
    Nacc = h0p->Integral(1, -1) * _eff * _lumi * _days * 24.0 * 3600.0 / _simdensity;
    Ntri = h1p->Integral(1, -1) * _eff * _lumi * _days * 24.0 * 3600.0 / _simdensity;
    std::cout << Nsim << "  " << Nacc << std::endl;
    if (Nacc > 1.0e+5 && Nsim > 10){
      fn = hnp->Integral(1,-1) / h0p->Integral(1,-1);
      xm = hxp->Integral(1,-1) / h0p->Integral(1,-1);
  ...
  for (int nb = 0; nb < Nbinm; nb++){
    ...
    TH1D * hsim = new TH1D("hsim", "hsim", 1, 0.0, M_PI);
    TH1D * h0m = new TH1D("h0m", "h0m", 180, 0.0, M_PI);
    TH1D * h1m = new TH1D("h1m", "h1m", 180, 0.0, M_PI);
    TH1D * hnm = new TH1D("hnm", "hnm", 1, 0.0, M_PI);
    TH1D * hxm = new TH1D("hxm", "hxm", 1, 0.0, M_PI);
    TH1D * hym = new TH1D("hym", "hym", 1, 0.0, M_PI);
    TH1D * hzm = new TH1D("hzm", "hzm", 1, 0.0, M_PI);
    ...
    for (double ie = 0; ie < Nevent; ie = ie + 1){
      ...
      hsim->Fill(std::abs(kin[5]), 1);
      h0m->Fill(std::abs(kin[5]), sigmaN[1]*acc_ele*acc_pion[1]);
      h1m->Fill(std::abs(kin[5]), sigmaN[1]*acc_ele*acc_pion[1]*factor);
      hnm->Fill(std::abs(kin[5]), sigman[1]*acc_ele*acc_pion[1]);
      hxm->Fill(std::abs(kin[5]), kin[0]*sigmaN[1]*acc_ele*acc_pion[1]);
      hym->Fill(std::abs(kin[5]), kin[1]*sigmaN[1]*acc_ele*acc_pion[1]);
      hzm->Fill(std::abs(kin[5]), kin[2]*sigmaN[1]*acc_ele*acc_pion[1]);
    ...
    Nsim = hsim->Integral(1, -1);
    Nacc = h0m->Integral(1, -1) *_eff * _lumi * _days * 24.0 * 3600.0 / _simdensity;
    Ntri = h1m->Integral(1, -1) *_eff * _lumi * _days * 24.0 * 3600.0 / _simdensity;
    std::cout << Nsim << "  " << Nacc << std::endl;
    if (Nacc > 1.0e+5 && Nsim > 10){
      fn = hnm->Integral(1, -1) / h0m->Integral(1, -1);
      xm = hxm->Integral(1, -1) / h0m->Integral(1, -1);
...
int Lanalysis::EDilutionProton(const char bintree[], const char savefile[]){
  ...
  for (int i = 0; i < Nbinp; i++){
    ...
    ENH3He4 = 0.03; 
    ErrRelpf = std::abs(ENH3He4 * RNH3He4 * 1.487 / (1.487 * RNH3He4 - 1.0));
  ...
  for (int i = 0; i < Nbinm; i++){
    ...
    //ENH3He4 = ((FF1[0] + 3.0 * FF3[0]) / FF2[0] - RNH3He4) / RNH3He4;
    ENH3He4 = 0.03;
    ErrRelpf = std::abs(ENH3He4 * RNH3He4 * 1.487 / (RNH3He4 * 1.487 - 1.0));
    Lstructure::FUUTN_bound(AZnitrogen, var, FF1, "nitrogen-14");
    Lstructure::FUUTN_bound(AZcarbon, var, FF2, "carbon-12");
...
int Lanalysis::ETotalProton(const char dir[], const char savefile[]){
  TString directory = "Proton/";
  TFile * fb11 = new TFile(directory+"sidisbin_11.root", "r");
  TFile * fb8 = new TFile(directory+"sidisbin_8.root", "r");
  TFile * fc11 = new TFile(directory+"sidiscoin_11.root", "r");
  TFile * fc8 = new TFile(directory+"sidiscoin_8.root", "r");
  TFile * fd11 = new TFile(directory+"sidisdilu_11.root", "r");
  TFile * fd8 = new TFile(directory+"sidisdilu_8.root", "r");
  //Set branch
  double BinNumber, Nbin;
  double Nucleon, Hadron, Ebeam;
  double x, y, z, Q2, Pt, fp;
  double A1, A2, A3;
  double E1stat, E2stat, E3stat;
  double E1rel, E2rel, E3rel, E1abs, E2abs, E3abs;
  double Err[3], tmp[3], temp1, temp2, temp3;
  ...
  Tb->SetBranchAddress("Q2m", &Q2);
  Tb->SetBranchAddress("Ptm", &Pt);
  Tb->SetBranchAddress("fp", &fp);
  Tb->SetBranchAddress("A1p", &A1);
  Tb->SetBranchAddress("A2p", &A2);
  Tb->SetBranchAddress("A3p", &A3);
  Tb->SetBranchAddress("E1stat", &Err[0]);
  Tb->SetBranchAddress("E2stat", &Err[1]);
  Tb->SetBranchAddress("E3stat", &Err[2]);
  Tc->SetBranchAddress("SB", &temp1);
  Tc->SetBranchAddress("ErrRel", &temp2);
  Td->SetBranchAddress("ErrRel", &temp3);
  std::cout << "Total error: 11GeV pi+" << std::endl;
  for(int i = 0; i < Nbin; i++){
    //statistics
    Tb->GetEntry(i);
    ...
    E3stat = Err[2] / _ST / fp;
    //raw asymmetry (square)
    E1abs = pow(0.02 / sqrt(660.0) / _ST / fp, 2);
    E2abs = pow(0.02 / sqrt(660.0) / _ST / fp, 2);
    E3abs = pow(0.02 / sqrt(660.0) / _ST / fp, 2);
    //target polarization (square)
    E1rel = 0.03*0.03;
    E2rel = 0.03*0.03;
    E3rel = 0.03*0.03;
    ...
    E2rel = E2rel + pow(0.2/(1.0 + temp1), 2);
    E3rel = E3rel + pow(0.2/(1.0 + temp1), 2);
    //dilution factor (square);
    Td->GetEntry(i);
    E1rel = E1rel + temp3*temp3;
    E2rel = E2rel + temp3*temp3;
    E3rel = E3rel + temp3*temp3;
    //radiative (square)
    E1rel = E1rel + 0.02*0.02;
    E2rel = E2rel + 0.02*0.02;
    E3rel = E3rel + 0.02*0.02;
  ...
  for(int i = 0; i < Nbin; i++){
    ...
    E3stat = Err[2] / _ST / fp;
    //raw asymmetry (square)
    E1abs = pow(0.02 / sqrt(330.0) / _ST / fp, 2);
    E2abs = pow(0.02 / sqrt(330.0) / _ST / fp, 2);
    E3abs = pow(0.02 / sqrt(330.0) / _ST / fp, 2);
    //target polarization (square)
    E1rel = 0.03*0.03;
    E2rel = 0.03*0.03;
    E3rel = 0.03*0.03;
