...
int main(int argc, char **argv)
{
    ...
    TString TARGET; 
    TString PARTICLE; 
    int ERROR;//0: STAT, 1: TOTAL, 2:SYST
    ...
    if (true){
      TString clasfile = "./data/clas_proton_results/projections_4D_pim_solid.out";
      ifstream infile(clasfile);
      if (infile.is_open()) cout << "open .. " << endl;
      TFile * fs = new TFile("clas_proton_pim.root","RECREATE");
      TTree * Ts = new TTree("data", "data");
      Ts->SetDirectory(fs);
      double bin, x, Pt, z, Q2, y, Nn, Estat, unpol, pol, Asym;
      double Ebeam = 11.0;
      double Erel = 0;
      double Eabs = 0;
      TString dum;
      ...
      Ts->Branch("unpol", &unpol, "unpol/D");
      Ts->Branch("pol", &pol, "pol/D");
      Ts->Branch("Asym", &Asym, "Asym/D");
      Ts->Branch("Estat", &Estat, "Estat/D");
      Ts->Branch("Erel", &Erel, "Erel/D");
      Ts->Branch("Eabs", &Eabs, "Eabs/D");
      infile >> dum >> dum >> dum >> dum >> dum >> dum >> dum >> dum;
      int nn = 0;
      while(!(infile.eof())){
	infile >> bin >> x >> Pt >> z >> Q2 >> y >> Nn >> Estat;
	if (Estat > 1.0e-9){
          cout << nn++ << endl;
	  unpol = UNPOLARISED_P_PIM(Ebeam, z, x, Q2, Pt);
	  pol = POLARISED_P_PIM(Ebeam, z, x, Q2, Pt);
